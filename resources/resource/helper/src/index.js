...
export default base =>
  class extends base {
    async create({typeOrSpecifier}, environment) {
      let type;
      let specifier;
      if (this.constructor.$isTypeIdentifier(typeOrSpecifier)) {
      ...
      await task(
        async () => {
          ...
          const existingResource = await this.constructor.$load(directory, {
    ...
    async _pinResource(specifier) {
      ...
      if (versionRange.toJSON() === undefined) {
        const resource = await this.constructor.$load(identifier);
        if (resource.version) {
