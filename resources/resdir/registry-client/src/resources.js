...
export default Resource => ({
    ...
    return await task(
      async progress => {
        const result = await root.authenticatedCall(
          accessToken => server.getResource({specifier, throwIfNotFound, accessToken}, environment),
          environment
        );
  ...
  async fetch({specifier}, environment) {
    const root = this.$getRoot();
    const resourceFetcher = await Resource.$getResourceFetcher();
  ...
  async publish({file, permissionToken}, environment) {
    ...
    const resource = await Resource.$load(file);
