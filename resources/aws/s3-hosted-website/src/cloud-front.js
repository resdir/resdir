      ...
      async () => {
        const params = {
          DistributionConfigWithTags: {
            ...
            Tags: {
              Items: [{Key: 'managed-by', Value: this.MANAGER_IDENTIFIER}]
            }
          }
        };
...
async function waitUntilCloudFrontDistributionIsDeployed(cloudFront, distributionId, environment) {
  await task(
    async () => {
      ...
      throw createClientError(
        `CloudFront deployment uncompleted after ${totalSleepTime / 1000} seconds`
      );
    },
    {
...
async function waitUntilCloudFrontInvalidationIsCompleted(
  ...
  await task(
    async () => {
      ...
      throw createClientError(
        `CloudFront invalidation uncompleted after ${totalSleepTime / 1000} seconds`
      );
    },
    {
