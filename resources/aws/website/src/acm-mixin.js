...
async function findACMCertificate(acm, domainName, managedByTag, {verbose, quiet, debug}) {
  ...
  return await task(
    async progress => {
      ...
      if (result.NextToken) {
        throw new Error(
          'Wow, you have a lot of ACM certificates! Unfortunately, this tool can\'t list them all. Please post an issue on Resdir\'s GitHub if this is a problem for you.'
        );
      }
